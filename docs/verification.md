...
- Native Scripting 0.4.4 (`Gen1Recomp Native 044`): 32 Einträge, 8.322.573 Bytes, SHA-256 `a718337d…`; setzt für Game-Sessions den upstream-dokumentierten Hostoverride `POKEPORT_TOUCH=1` und verwendet weiterhin LÖVE-Touchevents/Upstream-Overlay. Ein exakter Artefakt-Browserlauf bestätigte `POKEPORT_TOUCH=1`/Yellow in `Module.env` bei laufender Runtime. Der native Launch behält außerdem den stabilen Card-Tree statt des mit `t.__type__` korrelierten transienten Working-Rebuilds. Physische Touch-/UI-Regression steht aus. Bericht: [`gen1recomp/scripting-native-044-report.json`](gen1recomp/scripting-native-044-report.json);
