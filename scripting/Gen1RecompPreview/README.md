# Gen1Recomp Preview 0.1.4 (project identity: Gen1Recomp Preview 014)
...
This build intentionally provides something runnable before the full native launcher is implemented. Version 0.1.4 uses the distinct project name `Gen1Recomp Preview 014` and runtime directory `runtime-v014` so Scripting/iCloud cannot reuse previously imported files or WebView cache. Payload, Lua normalization scripts, and WASM are embedded as hash-pinned classic JavaScript data, avoiding the physical-device failure of `fetch(file://…)`.
