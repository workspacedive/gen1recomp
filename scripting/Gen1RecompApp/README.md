...
Native 0.4.0–0.4.4 physically proved canonical Yellow extraction, title/audio and ordinary upstream-touch play into Oak's Lab. Native 0.4.5 exposed the pre-presentation `1×1` viewport and repeated 50–105 ms frame gaps. Native 0.4.6 waited for stable visible geometry, filled the `440×956` WebView with a `640×1391` aspect-matched surface, and reduced late steady windows to p95 23–25 ms with no gaps over 50 ms. Direct logs showed that audible sound was correct while synchronous synthesis still consumed main-thread frame budget: 1024-sample music calls reached 23 ms and first-use effects reached 412 ms. Native 0.4.7 distributes music as 512-sample slices, caches rendered effects privately with exact signature validation, and measures update/draw CPU separately.
...
- Native 0.4.5 exposed the `1×1` bug; Native 0.4.6 physically proved full portrait geometry and major smoothing, then isolated residual music pacing and 106–412 ms first-use effects. Native 0.4.7 requires first-/second-run private-cache and update/draw evidence. Cache-ready UI, App Group durability, exact multi-touch/release, saves, lifecycle, iPad adaptation, and component/mod evidence remain open.
