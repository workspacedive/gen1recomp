...
- Native Scripting 0.4.5 (`Gen1Recomp Native 045`): 32 Einträge, 8.325.762 Bytes, SHA-256 `f0f73aa2…`; hostseitiger `conf.lua`-Wrapper plant eine aspect-treue, bounded Vollflächen-Surface (Telefon 4×, Tablet 5×) und belässt die lange Achse dem Upstream-Expanded-World-Renderer. Zehnsekündige RAF-/Long-Task-Rohtelemetrie misst das Stottern vor Optimierung. Zwei TypeScript-Ziele, 108 Node-Tests und fokussierte Python-/Determinismusprüfungen bestanden; der erneute exakte Browserlauf blieb wegen wiederholtem `ECONNRESET` beim Chromium-Download blockiert und wird nicht als bestanden behauptet. Bericht: [`gen1recomp/scripting-native-045-report.json`](gen1recomp/scripting-native-045-report.json);
