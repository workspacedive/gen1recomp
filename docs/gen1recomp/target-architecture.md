...
Physical WebView startup and Native 0.2.0 diagnostics are proven. Native 0.4.0–0.4.4 proved canonical Yellow extraction, title/audio and ordinary upstream-touch play into Oak's Lab. Native 0.4.5 then measured the pre-presentation `1×1` viewport that produced its square surface and severe periodic main-thread gaps matching the no-worker 8192-sample music cadence. Native 0.4.6 defers bundle startup to stable presented geometry and uses a separate audio adapter plus a two-line configurable ChipAudio seam to schedule unchanged 44.1 kHz synthesis in 1024-sample slices. Cache-ready state, App Group relaunch durability, exact multitouch/release, saves, close/flush, physical stutter correction, residual SFX/cry synthesis, broader fidelity, save management, mod activation, component/mod/GitHub operations and broad device validation remain active slices. The independent non-blocking Scripting `t.__type__` event remains open.
