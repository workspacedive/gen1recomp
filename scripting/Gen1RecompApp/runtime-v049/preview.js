...
function installGameChrome(hostSession) {
  ...
  if (chrome == null || title == null || close == null) return
  title.textContent = identities[hostSession.gameId].title
  chrome.hidden = false
  chrome.setAttribute("aria-hidden", "false")
  let fadeTimer = null
  const reveal = () => {
    chrome.dataset.faded = "false"
    if (fadeTimer != null) window.clearTimeout(fadeTimer)
    fadeTimer = window.setTimeout(() => { chrome.dataset.faded = "true" }, 3_200)
  }
  ...
  document.addEventListener("pointerdown", (event) => {
    if (event.clientY <= 48 && chrome.dataset.faded === "true") reveal()
  }, { capture: true })
  reveal()
