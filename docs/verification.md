...
- Native Scripting 0.4.1 (`Gen1Recomp Native 041`): 32 Einträge, 8.320.895 Bytes, SHA-256 `15cd150c…`; Patchkandidat installiert den bestehenden, differentialgetesteten BitOp-Shim vor Upstream-Start zusätzlich als `_G.bit`, ohne Gen1Recomp- oder LÖVE-Coreänderung. Die exakte ROM-freie Artefakt-Runtime mit Payload `a8a370be…` erreichte in Chromium 149 Frame 97 ohne Page-/Request-/HTTP-Fehler. Physische Yellow-Regression steht aus; erst sie kann abgeschlossene Extraktion belegen. Bericht: [`gen1recomp/scripting-native-041-report.json`](gen1recomp/scripting-native-041-report.json);
