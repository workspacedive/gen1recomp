...
| PUC Lua 5.1 via love.js | **Candidate** | third party reports slower than native/no LuaJIT | compatibility and browser limitations | Preferred feasibility spike because it preserves `.love`/LÖVE shape better than a rewrite. |
...
| WKWebView WebGL | **Candidate** | WebKit generally supports WebGL/WebGL2; Scripting exposes WebViewController | local-file, context, memory and lifecycle need device proof | Preferred graphics candidate for love.js spike. |
| WKWebView WebAssembly | **Candidate** | WebKit supports WebAssembly generally | same host/local/CSP proof required | Probe with a tiny bundled module. |
...
Proceed only with a **WebView + LÖVE 11.5 web-runtime feasibility spike**, while implementing Scripting platform UI and brokers as independent TypeScript components. `TimelineCanvas` is the diagnostic/fallback backend. Metal, native LuaJIT, raw controller input and low-level profiling remain unavailable or unverified and must not appear as implemented components.
