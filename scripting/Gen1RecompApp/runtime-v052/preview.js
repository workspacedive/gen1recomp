...
import {
  createLoveJsInterruptionRecovery,
} from "../../../runtime/adapters/lovejs/interruption.ts"
...
const identities = Object.freeze({
  red: { title: "Pokémon Red", size: 1048576, sha1: "ea9bcae617fdf159b045185467ae58b2e4a48b9a", saveSuffix: "" },
  blue: { title: "Pokémon Blue", size: 1048576, sha1: "d7037c83e1ae5b39bde3c30787637ba1d4c48ce2", saveSuffix: "_blue" },
  yellow: { title: "Pokémon Yellow", size: 1048576, sha1: "cc7d03262ebfaf2f06772c1a480c7d9d5f4a38e1", saveSuffix: "_yellow" },
  gold: { title: "Pokémon Gold", size: 2097152, sha1: "d8b8a3600a465308c9953dfa04f0081c05bdcb94", saveSuffix: "_gold" },
})
...
function installNativeViewport() {
  window.Module.env = {
    ...
    POKEPORT_TOUCH_VECTOR: "1",
    POKEPORT_TOUCH_PIXEL_SCALE: String(nativeViewport.devicePixelRatio),
  }
  bridge.log(
    "info",
...
function installGameChrome(hostSession) {
  if (hostSession == null) return null
  const chrome = document.getElementById("game-chrome")
  const title = document.getElementById("game-chrome-title")
  const close = document.getElementById("game-chrome-close")
  if (chrome == null || title == null || close == null) return null
  title.textContent = identities[hostSession.gameId].title
  chrome.dataset.faded = "true"
  let fadeTimer = null
  const reveal = (holdMs = 5_500) => {
    chrome.hidden = false
    chrome.setAttribute("aria-hidden", "false")
    window.requestAnimationFrame(() => { chrome.dataset.faded = "false" })
    bridge.log("info", `transient game title shown for ${holdMs}ms`)
  ...
  close.addEventListener("click", (event) => {
    ...
    void bridge.event({ type: "preview.dismiss", token: hostSession.token,
      gameId: hostSession.gameId })
  })
  document.addEventListener("pointerdown", (event) => {
    if (event.clientY <= 48 && chrome.dataset.faded === "true") reveal(3_200)
  }, { capture: true })
  bridge.log("info", "transient game title installed")
  return { reveal }
}
...
function installRomInjection(hostSession, bytes) {
  ...
  module.env = {
    ...
    POKEPORT_TOUCH: "1",
    // Native 046 removed the severe 8192-sample cadence, while direct CPU
    // logs showed smaller residual pacing and first-use effect stalls. Keep
    // identical 44.1 kHz synthesis, but use even slices and a private cache.
    POKEPORT_AUDIO_SLICE: "1",
    POKEPORT_AUDIO_CACHE: "1",
    POKEPORT_AUDIO_CACHE_NAMESPACE: "gen1recomp-0.1.96-chip-v1",
    POKEPORT_FRAME_TIMING: "1",
  }
...
function installPersistenceLifecycle(persistence) {
  ...
  const recovery = createLoveJsInterruptionRecovery({
    ...
    mainLoop: () => window.Module?.Browser?.mainLoop ?? null,
    flush,
    log: (level, message) => bridge.log(level, message),
    event: (value) => { void bridge.event(value) },
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") recovery.suspend("hidden")
    else recovery.resume("visible")
  })
  window.addEventListener("blur", () => recovery.suspend("blur"))
  window.addEventListener("focus", () => recovery.resume("focus"))
  window.addEventListener("pageshow", () => recovery.resume("pageshow"))
  window.addEventListener("pagehide", (event) => {
    flush("game-pagehide")
    recovery.pageHide(event.persisted)
    if (!event.persisted) {
      active = false
...
async function start() {
  ...
  const gameChrome = installGameChrome(hostSession)
  let romBytes = null
  if (hostSession != null) {
