...
Physical WebView startup and Native 0.2.0 diagnostics are proven. Native 0.4.0–0.4.2 proved canonical Yellow retention/extraction, generated-data/game/display loading and exact BitOp/queue-audio attribution. Native 0.4.3's narrow host guard then physically reached Yellow title/intro with immediately audible music and no generic modal. Touch controls remained hidden because love.js reports `Web`; Native 0.4.4 activates the existing upstream overlay with documented `POKEPORT_TOUCH=1`, still using LÖVE callbacks. It also removes the native modal-adjacent transient tree rebuild correlated with Scripting `t.__type__`. Cache-ready state, App Group relaunch durability, actual touch/multitouch, saves, close/flush, broader audio/fidelity, save management, mod activation, Native 0.3.0 component/mod/GitHub operations and broad device validation remain active slices.
