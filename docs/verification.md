...
**Letzter Lauf:** 2026-08-17T20:23:04Z
...
- Native Scripting 0.5.2 (`Gen1Recomp Native 052`): 33 Einträge, 8.345.737 Bytes, SHA-256 `e86c6b90…`; ersetzt ausschließlich legacy `tabIndex`/`tag`/`tabItem` durch die aktuelle offizielle App-Store-Struktur `TabView(selection: Observable)` mit fünf expliziten `Tab`-Deskriptoren und je einem `NavigationStack`. Beide TypeScript-Ziele, 122/122 Node- und 30 ausgeführte Python-Tests (5 Umgebungsskips), deterministischer Rebuild, ZIP und npm audit 0 bestanden; physische Nichtwiederkehr wird nicht statisch behauptet. Bericht: [`gen1recomp/scripting-native-052-report.json`](gen1recomp/scripting-native-052-report.json);
...
- physische Scripting-/iOS-Ausführung: Preview 0.1.4 und Native-0.2.0-Settings→Runtime-Startup **bestanden** auf iPhone/iOS 18.7; Native 0.4.0–0.5.1 belegten auf Scripting 3.2.0/iOS 26.6 kanonischen Yellow-Import, Extraktion, Spiel, Touch, Retina, Pacing und readiness-gebundenen Titel. `t.__type__` blieb in 0.5.1 bestehen. Native 0.5.2 sowie Datei-/GitHub-Mod-, Komponenten- und Save-Backup-Pfade bleiben offen.
...
- Native-0.5.2-moderne Fünf-Tab-TSX, `t.__type__`-Nichtwiederkehr und adaptive Game-/Save-Karten auf iPhone/iPad einschließlich Dark Mode, Dynamic Type und VoiceOver;
