...
Physical runs through Native 0.4.6 prove canonical Yellow import/play, full `440×956` presented geometry and major removal of severe periodic stalls. Direct timing then isolated smaller 7–23 ms music calls and 106–412 ms first-use static effects despite correct continuous audible sound. Native 0.4.7 spreads unchanged 44.1 kHz work into 512-sample slices, stores rendered effects only in the private removable game cache with exact full-signature validation, and measures update/draw CPU separately. Warm-cache performance, cache-ready state, App Group relaunch durability, exact multitouch/release, saves, broader fidelity, mod activation, component/mod/GitHub operations and broad-device validation remain active slices. The independent non-blocking Scripting `t.__type__` event remains open.
