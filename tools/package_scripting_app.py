...
DEFAULT_OUTPUT = REPO_ROOT / "research" / "downloads" / "gen1recomp" / "Gen1Recomp Native 051.scripting"
PACKAGED_RUNTIME = "runtime-v051"
RUNTIME_SHELL = "runtime-shell-v051"
...
def collect_entries(source: Path, launcher: Path, lock_path: Path) -> dict[str, bytes]:
    ...
    if metadata["name"] != "Gen1Recomp Native 051" or metadata["version"] != "0.5.1":
...
def package(source: Path, launcher: Path, lock_path: Path, output: Path) -> dict[str, object]:
    ...
    return {
        **report,
        ...
        "scope": "native verified-ROM library, transactional save backup/restore/delete and slot launch, Retina/vector runtime, post-ready game chrome, interruption recovery, cache, updates and mods; physical 0.5.1 verification pending",
    }
