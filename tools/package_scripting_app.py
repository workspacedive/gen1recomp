...
DEFAULT_OUTPUT = REPO_ROOT / "research" / "downloads" / "gen1recomp" / "Gen1Recomp Native 047.scripting"
PACKAGED_RUNTIME = "runtime-v047"
RUNTIME_SHELL = "runtime-shell-v047"
...
def collect_entries(source: Path, launcher: Path, lock_path: Path) -> dict[str, bytes]:
    ...
    if metadata["name"] != "Gen1Recomp Native 047" or metadata["version"] != "0.4.7":
...
def package(source: Path, launcher: Path, lock_path: Path, output: Path) -> dict[str, object]:
    ...
    return {
        **report,
        ...
        "scope": "native verified-ROM library, presented viewport, even audio scheduling, private rendered-effect cache, frame timing, manual components and mods; physical 0.4.7 verification pending",
    }
