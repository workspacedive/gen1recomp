...
**Letzter Lauf:** 2026-08-16T20:08:41Z
...
- Scripting-Phase-0-Paket: ebenfalls auf dokumentierten `Script`-Import und klassisches Safari-13-kompatibles Bundle mit diagnostischem Loader korrigiert; deterministisches ROM-freies Archiv mit 14 Einträgen, 7.598.818 Bytes und SHA-256 `7e8b6342…`; die exakt extrahierte Runtime bestand in Chromium Boot, Suspend-Flush (Frame 42 stabil), Resume (Frame 72) und Stop-Flush/Dispose ohne Fehler;
- Interaktive Scripting-Preview 0.1.3 (`Gen1Recomp Preview 013`): nach dem ersten echten Gerätebefund (`Script` nicht global; lokale ES-Module starteten nicht) auf dokumentierten `Script`-Import und ein einzelnes klassisches esbuild-Bundle korrigiert. Nach dem dritten Bericht mit nachweislich alten Log-Literalen verwendet das Paket zusätzlich eine neue Projektidentität und `runtime-v013`-URL. Das diagnostische Safari-13-Bundle entfernt private Felder und optionale Verkettung; das deterministische ROM-freie Archiv hat 14 Einträge, 7.599.148 Bytes und SHA-256 `a9f69758…`; die exakt extrahierte Preview erreichte erneut den sichtbaren Launcher, erzeugte Console-/Ready-Diagnostik und schloss per explizitem Flush mit `Module.done=true` ohne Page-, Request- oder Runtime-Fehler. Beide `index.tsx` bestanden strikte TypeScript-Diagnostik gegen einen temporären, aus der offiziellen Dokumentation übertragenen WebViewController-/FileManager-/Script-Signaturensatz; app-synchronisierte `.d.ts`-Diagnostik und physische Scripting-Ausführung sind weiterhin offen;
