...
**Letzter Lauf:** 2026-08-17T20:48:52Z
...
- Native Scripting 0.4.6 (`Gen1Recomp Native 046`): physisch korrekte `440×956` Vollfläche (`640×1391` backing) und starke Glättung; drei späte Fenster p95 23–25 ms, Max 31–37 ms, null Gaps >50 ms. Direkte CPU-Messung isolierte 7–23 ms Musik-Slices und 106–412 ms erstmals erzeugte Effekte. Bericht: [`gen1recomp/scripting-native-046-report.json`](gen1recomp/scripting-native-046-report.json);
- Native Scripting 0.4.7 (`Gen1Recomp Native 047`): physisch deutlich flüssiger; bestes Fenster 601 Frames/10,017 s, p95 20 ms, Max 24 ms. Draw-CPU spät 0,15–0,33 ms; benannte Erst-Effekte 13–358 ms wurden privat gespeichert. Bericht: [`gen1recomp/scripting-native-047-report.json`](gen1recomp/scripting-native-047-report.json);
- Native Scripting 0.4.8 (`Gen1Recomp Native 048`): physisch exakt `1320×2868`/Scale 8, spätes Draw 0,09–0,37 ms und stabile p95 um 23 ms; Vektoradapter installiert. Telefon-Recovery/Cache-Hit blieben ungetestet. Bericht: [`gen1recomp/scripting-native-048-report.json`](gen1recomp/scripting-native-048-report.json);
- Native Scripting 0.4.9 (`Gen1Recomp Native 049`): nativer Titel entfernt, Custom-Fade vor sichtbarem Gameplay beendet und `t.__type__` trotz direkter Toolbar-Buttons erneut; Bericht: [`gen1recomp/scripting-native-049-report.json`](gen1recomp/scripting-native-049-report.json);
- Native Scripting 0.5.0 (`Gen1Recomp Native 050`): 32 Einträge, 8.336.003 Bytes, SHA-256 `dba920d6…`; zeigt Chrome erst post-ready 5,5 s und blendet 0,9 s ästhetisch aus. Root-TabView besitzt nun fünf unmittelbare native getaggte NavigationStacks statt Custom-Screen-Deskriptoren. 112 Node- und 30 ausgeführte Python-Tests sowie deterministischer Rebuild/npm audit 0 bestanden; physische Regression steht aus. Bericht: [`gen1recomp/scripting-native-050-report.json`](gen1recomp/scripting-native-050-report.json);
- Native Scripting 0.5.1 (`Gen1Recomp Native 051`): 33 Einträge, 8.345.651 Bytes, SHA-256 `6ab5468a…`; ergänzt Metadaten-Refresh, exakten `--slot`-Start, geschlossenes SHA-256-Save-Backup über die Dateien-App, doppelt validierten transaktionalen Restore mit `.bak`-Erhalt und bestätigtes selektives Löschen. Physisch bestanden Runtime-Identität, readiness-gebundener 5,5-s-Titel, Frame 8, Yellow-Spielstart und Retina `1320×2868`; `t.__type__` trat weiter einmal auf und widerlegt den abgeflachten Legacy-Tab-Baum als vollständige Korrektur. Save-Aktionen blieben offen. Bericht: [`gen1recomp/scripting-native-051-report.json`](gen1recomp/scripting-native-051-report.json);
- Native Scripting 0.5.2 (`Gen1Recomp Native 052`): 33 Einträge, 8.345.737 Bytes, SHA-256 `e86c6b90…`; moderne fünf Tabs renderten physisch korrekt, Yellow/Frame 8/Retina blieben erhalten, aber `t.__type__` trat erneut einmal auf. Der Screenshot deckte zusätzlich die falsche Gleichsetzung von null Ready-Spielen mit leerer Registry auf. Bericht: [`gen1recomp/scripting-native-052-report.json`](gen1recomp/scripting-native-052-report.json);
- Native Scripting 0.5.3 (`Gen1Recomp Native 053`): 33 Einträge, 8.346.167 Bytes, SHA-256 `3cdef5e2…`; entfernt alle Toolbar-Deskriptoren, zentralisiert einen Dismiss-Hook, bietet fünf gewöhnliche Close-Buttons und modelliert Home als Loading/Content/Error mit getrennten Ready/Pending/Reimport-Zählungen. Beide TypeScript-Ziele, 123/123 Node- und 30 ausgeführte Python-Tests (5 Umgebungsskips), deterministischer Rebuild, ZIP und npm audit 0 bestanden; physische Toolbar-Isolation bleibt offen. Bericht: [`gen1recomp/scripting-native-053-report.json`](gen1recomp/scripting-native-053-report.json);
...
- physische Scripting-/iOS-Ausführung: Preview 0.1.4 und Native-0.2.0-Settings→Runtime-Startup **bestanden** auf iPhone/iOS 18.7; Native 0.4.0–0.5.2 belegten auf Scripting 3.2.0/iOS 26.6 kanonischen Yellow-Import, Extraktion, Spiel, Touch, Retina, Pacing, readiness-gebundenen Titel und moderne Tabs. `t.__type__` blieb auch in 0.5.2 bestehen. Native 0.5.3 sowie Datei-/GitHub-Mod-, Komponenten- und Save-Backup-Pfade bleiben offen.
...
- Native-0.5.3-Toolbar-Isolation, zentraler/in-content Dismiss, korrekter Yellow-Home-Status und adaptive Game-/Save-Karten auf iPhone/iPad einschließlich Dark Mode, Dynamic Type und VoiceOver;
- DocumentPicker-Save-Export/Restore sowie Komponenten-/Modimport, GitHub API/Release-Asset-Redirect, App-Group-Registry/Persistenz, Unterbrechungs-Recovery und materialisierte Updates auf dem Gerät;
- periodischer/Visibility/Pagehide-/Exit-Flush und Save-Dauerhaftigkeit unter realen Dismiss-/Hintergrund-/Abbruchsequenzen;
- ausgewählter Slot-Start, Backup/Restore/Delete, Mods und reale Spielparitätsgoldens; ROM-Identität, Extraktion, vollständiger privater Cache und direkter Yellow-Spielstart sind bereits physisch belegt;
