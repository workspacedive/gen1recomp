...
The extended run passed 23/23 checks, adding a deterministic 1/60 accumulator, reload marker, coroutine, generated queueable-audio buffer and thread-channel roundtrip. `love.thread.newThread` exists as a symbol but worker construction fails in the tested love.js normalization layer. The generated pre-core wrapper hides that false capability; Fetch, Update, Mod Job, ROM-import coroutine and synchronous ChipAudio fallback selection now pass host-side characterization, while their device performance remains unproven. An immediate reload lost the marker in one Chrome 92 trial but recovered it in the current isolated Chromium 149 trial; explicitly awaiting `Module.FS.syncfs(false)` recovered it consistently. This timing/environment variance is itself the finding. `runtime/adapters/lovejs/persistence.ts` now serializes that explicit barrier, but only a Scripting suspend/exit/relaunch probe can establish host durability.
...
The compiled production stack was subsequently exercised through `LoveJsRuntimePort`, `LoveJsPersistenceAdapter`, and `BrowserLoveJsGameSurface`, rather than a duplicate probe-only lifecycle. With the exact deterministic launcher, boot reached the requested payload (not love.js' silent `nogame.love` fallback), suspend held frame 26 and completed explicit sync, resume advanced to frame 56, and controlled stop completed a second sync plus asynchronous exit without advancing again. The final Chromium run was cross-origin isolated and removed package-provided web-security-disabling flags; see [`lovejs-runtime-surface-report.md`](lovejs-runtime-surface-report.md). This is still outside-Scripting evidence only.
