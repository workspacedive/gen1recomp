...
| WKWebView WebGL | **Candidate** | WebGL1/2 and a minimal LÖVE shader passed in an outside-Scripting Headless Chrome 92/SwiftShader run; WebKit generally supports WebGL/WebGL2 | local-file, hardware context, memory and lifecycle still need Scripting device proof | Preferred graphics candidate for love.js spike. Do not transfer the software-browser result to iOS performance. |
