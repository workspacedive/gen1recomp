...
Native 0.4.0–0.4.8 physically proved canonical Yellow play, full Retina `1320×2868` output, fit scale 8 and near-60 Hz steady motion with negligible draw CPU. Native 0.4.9 removes permanent native game-title chrome in favor of a timed WebView fade/reveal/close surface. It also replaces five custom toolbar wrapper values with direct native Buttons matching Scripting's documented descriptor contract, targeting the recurring pre-runtime `t.__type__` failure without changing the core payload.
...
- Native 0.4.5–0.4.8 physically progressed from viewport attribution to Retina geometry and near-60 Hz motion. Native 0.4.9 requires transient-title/native-dismiss and direct-toolbar `t.__type__` regression evidence. Warm cache, incoming-call recovery, cache-ready UI, saves, exact multitouch/release, wider devices, components and mods remain open.
